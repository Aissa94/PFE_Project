...
//**************************   IMAGE BINARIZATION *********************************
//---------------------------------------------------------------------------------
cv::Mat Image_processing::Image_Binarisation(cv::Mat img, double threshold){
/*
...
//---------------------------Print Segmentation -------------------------------
cv::Mat Image_processing::Print_Segmentation(cv::Mat img){
            ...
            if ((theta >=0)&&(theta<=PI_4)){
                ...
                if ((theta>PI_4)&&(theta<=PI_2)){
                    ...
                    if ((theta <0)&&(theta>=-PI_4)){
                        ...
                        else{ a.x= blc_size*j+mi_blc_size;
...
//**************************   IMAGE THINNING *************************************
//---------------------------------------------------------------------------------
cv::Mat Image_processing::Image_Thinnig(cv::Mat img, bool black){
...
//**************************   MINUTIA EXTRACTION  ********************************
//---------------------------------------------------------------------------------
Minutiae_Features Image_processing::Minutia_Extraction(cv::Mat img, cv::Mat oimg, int bloc_size){
/*
 * img : thinned image
 */
...
void Image_processing::Draw_minutiae_Features (bool col,cv::Mat &img, Minutiae_Features minutiae){
    ...
    cv::Point2d a,b;
    cv::Scalar color_cercle(255,255,0);
...
void Image_processing::Minutiae_Validation (cv::Mat img,Minutiae_Features &minutiae, cv::Mat msk ){
...
// +++++++++++++++++ Segmentation ++++++++++++++
  ...
  //    cv::imshow("image Segmented",_msk);
    ...
    msk=1-msk;
    //cv::bitwise_or(imgb,msk,imgb);    // if not commented it will not work !!!!!!!!!!!!!!!!  
// +++++++++++++++++ Thining +++++++++++++++++++
...
//    Draw_minutiae_Features(_imgt2,minutiae);
...
cv::Mat Image_processing::thinning(cv::Mat input, cv::Mat &enhancedImage, cv::Mat &segmentedImage){
	...
	try{
		// Run algorithms
		enhancedImage = imageProcessing.Image_Enhancement(input, 16, 4, 4, 11);
		binarizedImage = imageProcessing.Image_Binarisation(enhancedImage, 0.48);
		segmentedImage = imageProcessing.Print_Segmentation(input);
	}
	...
	//+++++++++++++++++++ Thining+++++++++++++++++++++++++++++++++++
	thinnedImage = imageProcessing.Image_Thinnig(binarizedImage, true);
